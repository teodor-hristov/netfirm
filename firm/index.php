...
<body style="overflow: hidden;">
    ...
    <!-- Header -->
    <a name="about"></a>
        <video style="position: fixed; z-index: -800;  width: auto; height: auto;" autoplay loop>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="intro-message">
                        <h1 style="color: white;" class="animated bounceIn">NetFirm</h1>
      ...
      <div style="border-radius: 0px; background-color: #e5e2de;" class="modal-content">
        ...
        <div style="background-color:#f1f1f1" class="modal-footer">
            ...
            if(isset($_POST['register']))
            {
                ...
                $q = mysql_query("SELECT * FROM `users` WHERE `email` = '$regmail' AND `rank` = '1'");
                ...
                if(filter_var($regmail, FILTER_VALIDATE_EMAIL))
                {
                    if($regpass == $regconf)
                    {
                        if($check == 0)
                        {
                        mysql_query("INSERT INTO `users` (`email`, `password`,`rank`) VALUES('$regmail', '$regpass', '1')");
                        $_SESSION['user']['email'] = $regmail;
                        $_SESSION['user']['id'] = $arrayid + 1;
                        $_SESSION['user']['password'] = $regpass;
...
<div class="modal fade" id="login" role="dialog">
    <div class="modal-dialog modal-md">
      <div style="border-radius: 0px; background-color: #e5e2de;" class="modal-content">
        ...
        $q = mysql_query("SELECT * FROM `users` WHERE `email` = '$mail' AND `rank` = '1'");
        $check = mysql_num_rows($q);
